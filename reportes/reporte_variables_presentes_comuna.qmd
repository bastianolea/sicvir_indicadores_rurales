...
missing_comunas <- sicvir |>
  ...
  ungroup() |> 
  select(nombre_region, nombre_comuna, clasificacion, n_na, n_variables) |> 
  mutate(p_missing = n_na/n_variables,
         p_presentes = 1-p_missing)

# calcular por regiones
missing_regiones <- missing_comunas |> 
  group_by(nombre_region, clasificacion) |> 
  summarize(n_na = sum(n_na),
            n_variables = sum(n_variables),
            n_comunas = n()) |> 
  ungroup() |> 
  mutate(nombre_region = factor(nombre_region, rev(unique(sicvir$nombre_region)))) |> 
  mutate(clasificacion_pndr = factor(clasificacion, 
...
missing_regiones_2 |> 
  ggplot() +
  aes(valor, y = nombre_region, fill = estado) +
...
missing_regiones |> 
  ggplot() +
  aes(n_na, y = nombre_region, fill = clasificacion_pndr) +
  geom_col(position = position_stack(0.5),
           linewidth = 0.8, color = "white") +
...
outputs_comunas <- future_map(sicvir$nombre_comuna |> set_names(), \(.comuna) {
  ...
  region <- unique(variables_comuna$nombre_region)
  orden <- unique(variables_comuna$n)
  clasificacion_pndr <- unique(variables_comuna$clasificacion)
